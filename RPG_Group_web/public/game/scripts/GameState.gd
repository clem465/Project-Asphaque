...
func load_state() -> void:
	if _auth_token == "" or _api_base_url == "":
		print("[GameState] load_state: missing token or API URL")
		return
	print("[GameState] load_state: sending GET to", _api_base_url + "/state")
	_ensure_state_http()
...
func _post_state() -> void:
	if _auth_token == "" or _api_base_url == "":
		print("[GameState] _post_state: missing token or API URL")
		return
	var payload := {
		"state": _build_state_payload()
	}
	var body := JSON.stringify(payload)
	print("[GameState] _post_state: sending POST to", _api_base_url + "/state", "with body:", body)
	_ensure_state_http()
...
func _on_state_request_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	print("[GameState] _on_state_request_completed: mode=", _state_request_mode, "result=", result, "code=", response_code)
