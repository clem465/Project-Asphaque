...
1. Recommended: start API + web with one command:

```powershell
.\run-all.cmd
```

First run only (if Python deps are missing):

```powershell
.\run-all.cmd -InstallApiDeps
```

2. If you prefer manual startup, start the Python auth backend (login/register API) first:
...
3. Start Symfony + Docker services:

```powershell
.\run-local.cmd
```

4. Export Godot in Web format and publish it into the Symfony iframe slot:
...
5. Open the website (port shown by bootstrap, usually `8080` or `8081`).
...
By default, the app is exposed on host port `8080`. If that port is busy,
the bootstrap script automatically switches to the next available port.

Why this matters: the Python API uses `127.0.0.1:8000`, so keeping the web app
on `8080+` avoids API/web collisions that can produce `JSON invalide` in Godot.
...
2. MySQL credentials are machine-dependent (the backend now has SQLite fallback for local login, but DB provisioning is not fully standardized).
3. Godot publish is still a separate manual command after export.

## JSON error troubleshooting (team setups)

If someone sees `JSON invalide`, verify they are not calling Symfony `/login`
(HTML page) instead of the Python `/login` API (JSON response).

Checklist:

1. Python API up on `http://127.0.0.1:8000`
2. Web app up on `http://127.0.0.1:8080` (or next free port shown by bootstrap)
3. Optional explicit override in browser URL:
