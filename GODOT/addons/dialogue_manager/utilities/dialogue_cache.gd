...
static func prepare() -> void:
	...
	if main_loop and main_loop.root:
		main_loop.root.add_child(_update_dependency_timer)
...
static func reimport_files(and_files: PackedStringArray = []) -> void:
	...
	# Editor API is not available in exported builds; keep this safe and non-fatal.
	_files_marked_for_reimport.clear()
...
static func _schedule_deferred_reimport() -> void:
	...
	# Export/runtime path: no-op to avoid editor-only API calls.
	_files_marked_for_reimport.clear()
